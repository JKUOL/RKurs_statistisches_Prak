...
avg_RR_NKE <- mean(return_rate_NKE)
...
avg_RR_Sp500 <- mean(return_rate_SP500)
...
avg_RR_adi <- mean(return_rate_adi)
...
Years <- year(c(start_date,year1,year2,year3,year4,lastdate))

return_df <- bind_cols(Years, m_return_NKE, m_return_SP500, m_return_adi, m_return_ua)
...
return_to_date <- t(round(return_to_date[2:5]-1000))
colnames(return_to_date) <- "return in $"
...
print(return_plot <- mel_return_df %>%
  ...
  annotation_custom(tableGrob(return_to_date), xmin=2017.1, xmax=2018, ymin=2000, ymax=3550)
)
